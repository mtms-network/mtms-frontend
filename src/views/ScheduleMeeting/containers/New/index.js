...
const ScheduleMeetingItem = ({ t }) => {
    ...
    const [loop, setLoop] = useState('daily');
    ...
    useEffect(() => {
        if (meetingStore?.meetingTypes?.length && type === null) {
            setType(meetingStore.meetingTypes[0].uuid);
        }
    }, [meetingStore?.meetingTypes]);
                    ...
                    <div className="w-[90%] m-auto bg-white rounded-[20px] md:w-[80%] lx:w-[60%]">
                        <form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <GroupLayout className="flex flex-wrap gap-[12px]">
                                {meetingStore?.meetingTypes &&
                                    meetingStore.meetingTypes.map((item, key) => {
                                        return (
                                            <span
                                                key={key}
