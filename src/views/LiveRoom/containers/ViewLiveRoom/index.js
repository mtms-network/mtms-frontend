...
const ViewLiveRoom = ({ t }) => {
  ...
  const [maxInvites, setMaxInvinces] = useState(window.innerWidth > 400 ? 10 : 6);
  ...
  useEffect(() => {
    window.addEventListener('resize', () => {
      setMaxInvinces(window.innerWidth > 400 ? 10 : 6)
    });
      ...
      <GroupLayout className="flex flex-col justify-between">
        ...
        <div className="flex mb-6 flex-wrap items-center">
          ...
          <Button
            className="btn btn-outline btn-primary rounded-5 h-10 min-h-10 !mt-0 !mb-4 !mr-4"
            onClick={() => {
              navigate(`/${routeUrls.liveRoom.path}/${meetingStore?.meeting?.uuid}/edit`);
            }}
          >
            {t("general.edit")}
          </Button>
