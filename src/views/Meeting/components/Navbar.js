...
const OverviewNavBar = ({ t }) => {
  return (
    <div className="flex justify-center sm:justify-between items-center py-2">
      ...
      >
        <button className="btn btn-primary btn-sm px-8 sm:px-4 text-black">{t("home.wallet")}</button>
        <button className="btn btn-primary btn-sm px-8 sm:px-4 text-black">{t("home.user_center")}</button>
      </div>
    </div>
  );
};
